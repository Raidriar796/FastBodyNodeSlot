...
public partial class FluxCaching : ResoniteMod
{
    ...
    {
        [HarmonyPatch(typeof(FindParentByName), "Compute")]
        private class FindParentByNamePatch
        {
            private static bool Prefix(ref FrooxEngineContext context, FindParentByName __instance, ref Slot __result)
            {
                // Run original method if the mod is disabled
                if (!Config!.GetValue(enable) || !Config.GetValue(findParentByNameCaching)) return true;
